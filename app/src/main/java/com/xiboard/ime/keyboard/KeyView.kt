...
import com.xiboard.daemon.RimeDaemon
...
import com.xiboard.ime.popup.PopupAction
...
class KeyView(
    ...
    fun setPressedState(pressed: Boolean) {
        if (keyPressed != pressed) {
            ...
            if (pressed) {
                key.onPressed()
                runKeyPressAnimation(true)
            } else {
                key.onReleased()
                runKeyPressAnimation(false)
            }
            invalidate()
        }
    }

    private fun runKeyPressAnimation(pressed: Boolean) {
        pivotX = width / 2f
        pivotY = height / 2f
        animate().cancel()
        animate()
            .scaleX(if (pressed) KEY_PRESS_SCALE else 1f)
            .scaleY(if (pressed) KEY_PRESS_SCALE else 1f)
            .setDuration(if (pressed) KEY_PRESS_ANIM_IN_MS else KEY_PRESS_ANIM_OUT_MS)
            .start()
    }
    ...
    companion object {
        private const val KEY_PRESS_SCALE = 1.12f
        private const val KEY_PRESS_ANIM_IN_MS = 80L
        private const val KEY_PRESS_ANIM_OUT_MS = 90L
    }
}
